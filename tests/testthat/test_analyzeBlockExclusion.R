...
test_that("analyze block exclusions", {
  out <- analyzeBlockExclusion(processedObj, exclusionTuples)
...
test_that("analyze block exclusions errors", {
  ...
  expect_error(analyzeBlockExclusion(processedObj, exclusionTuples2), "Currently analyzeBlockExclusion only works if item pool is depleted.")
})
