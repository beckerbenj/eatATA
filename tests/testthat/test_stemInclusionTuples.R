...
load("helper_test_stemInclusionTuples.RData")
...
test_that("function works as intended with tibbles or data tables input instead of 'items' data frames", {
  # tibbles
  out <- stemInclusionTuples(stemInclusionTupl$inclDF_tibble, idCol = "ID", stemCol = "stem")
  expect_equal(out[, 1], paste0("item_", c(1, 1, 2, 4)))
  expect_equal(out[, 2], paste0("item_", c(2, 3, 3, 6)))

  # data tables
  out <- stemInclusionTuples(stemInclusionTupl$inclDF_dt, idCol = "ID", stemCol = "stem")
  expect_equal(out[, 1], paste0("item_", c(1, 1, 2, 4)))
  expect_equal(out[, 2], paste0("item_", c(2, 3, 3, 6)))
})
