...
#' ## When desired, the automatically computed range can be adjusted by hand. This can be of use when only a limited set of the categories should be constrained. For instance, when only the multiple-choice items should be constrained, and the non-multiple-choice items should not be constrained, the minimum and maximum value can be set to a very small and a very high value, respectively. Or to other sensible values.
