...
## Setup
The `eatATA` package can be installed from `CRAN`. As a default solver, we recommend `GLPK`, which is automatically installed alongside this package.   
...
Next, the objective function is defined: The TIF should be maximized at medium ability. For this, we use the `maxConstraint()` function. Other available constraint functions are `minConstraint()`, `maximinConstraint()`, `minimaxConstraint()`, and `cappedMaximinConstraint()`.
...
To automatically assemble the test form based on our constraints, we call the `useSolver()` function. In this function we define which solver should be used as back end.
